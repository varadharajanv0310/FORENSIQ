...
import { analyzeDocument, applyAdversarial, fileToBase64 } from '../services/api.js';
...
export function AnalysisProvider({ children }) {
  ...
  const runAdversarial = useCallback(async (operation, intensity) => {
    ...
    setStatus('loading');
    setError(null);
    const p = applyAdversarial(lastBase64Ref.current, operation, intensity);
    animateSteps(p);
    try {
      const data = await p;
      setResult(data);
  ...
  const value = useMemo(() => ({
    ...
    runAnalyze, runAdversarial, retry, reset,
  }), [file, result, baseline, status, error, stepIndex, runAnalyze, runAdversarial, retry, reset]);
